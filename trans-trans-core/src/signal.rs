...
impl<F: FileHandler> Onset<F> {
    ...
    /// read that loops over pcm block if `self.onset_start` is `Some`
    ...
    pub fn seek_from_start(&mut self, tick: i32, ticks_per_beat: u32, fs: &mut F) -> Result<(), F::Error> {
        let seek_to = self.onset_start.unwrap_or(0) as i64 * 2 + ((self.pcm_len as f32 / (self.beat_count * ticks_per_beat) as f32 * tick as f32) as i64 & !1);
...
pub struct SignalHandler<const LAYER_COUNT: usize, F: FileHandler> {
    ...
    inputs: [Option<state::OnsetInput>; LAYER_COUNT],
...
impl<const LAYER_COUNT: usize, F: FileHandler> SignalHandler<LAYER_COUNT, F> {
    ...
    /// if returning early, returns (should tick midi, should tick state, number of samples read)
    pub fn read<const ONSET_COUNT: usize>(&mut self, buffer: &mut [f32], channels: usize, sample_rate: u32, fs: &mut F) -> Result<Option<(bool, bool, usize)>, F::Error> {
    ...
    pub fn tick(&mut self, message: Option<state::Message<LAYER_COUNT>>, fs: &mut F) -> Result<Message<LAYER_COUNT>, Error<F::Error>> {
        if let Some(message) = message {
            self.ticks_per_beat = message.ticks_per_beat;
            self.ticks_per_meas = message.ticks_per_meas;
            self.inputs = message.inputs;
            for l in 0..LAYER_COUNT {
                ...
                if let state::OnsetEvent::Hold { onset, .. } | state::OnsetEvent::Loop { onset, .. } = &mut self.active_events[l] {
                    self.grains[l].fade::<F>(Some(onset), fs)?;
                } else if message.events[l].inner.is_some() {
                    self.grains[l].fade::<F>(None, fs)?;
                }
                // replace event
